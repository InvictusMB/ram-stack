...
module.exports = createMacro(materialUiMacro);

function materialUiMacro({references, babel}) {
  const {types} = babel;
  ...
  forEach(references, (referenceInstances, referenceName) => {
    if (referenceName === 'default') {
      ...
      throw new TypeError(`Not implemented yet. Use "import MUI from './material-ui.macro'" for now`);
    }
  });
}
...
function insertImport(reference, types, name) {
  ...
  const ast = types.importDeclaration(
    ...
    types.stringLiteral(`@material-ui/core/${name}`),
  );
